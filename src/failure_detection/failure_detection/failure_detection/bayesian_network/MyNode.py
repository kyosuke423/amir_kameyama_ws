...
class NodeSet:
    ...
    def initialize(self):
        ...
        self.node_dict["undetected_error_by_yolo"] = DiscreteDistribution({"True": 0.2, "False": 0.8}) #   認識
        ...
        self.solution_dict["obj_occlusion"] = "movement_of_viewpoint" #   視点移動したい
        ...
        ##########Pick動作関連###########################################################
        self.node_dict["pick_failure"] =  DiscreteDistribution({"not_failure": 0.5, "failure": 0.5}) #   planning以外の失敗.place


        ##########Place動作関連###########################################################
        self.node_dict["place_failure"] =  DiscreteDistribution({"not_failure": 0.5, "failure": 0.5}) #   planning以外の失敗.place
    ...
    def update(self):    
        ...
        self.node_dict["target_not_exist_by_arm_collision"] = ConditionalProbabilityTable(  #   衝突してもそんなに移動してなければ存在する判定でいいはず
            [["collision", "True", 0.9],
            ["collision", "False", 0.1],
        ...
        #   
        # self.node_dict["drop_obj"] =  DiscreteDistribution({"failed": 0.3, "not_failed": 0.6}) #   planning以外の失敗.pickを実行してなければnot

        # self.node_dict["target_not_exist_by_drop_obj"] = ConditionalProbabilityTable(  #   衝突してもそんなに移動してなければ存在する判定でいいはず
        #     [["failed", "True", 0.9],
        #     ["failed", "False", 0.1],
        #     ["not_failed", "True", 0.0],
        #     ["not_failed", "False", 1.0]],
        #     [self.node_dict["pick_failed"]])
        ...
        "---------------オブジェクト関連----------------" 
        # self.node_dict["target_not_exist"] = self.generate_OrCPT(["target_not_exist_by_arm_collision", "target_not_exist_by_invalid_location", "target_not_exist_by_img_false_detection"], ["exist", "not_exist"])


        #   placeに失敗したことでどこかにいく確率
        self.node_dict["target_not_exist_by_place_failure"] = ConditionalProbabilityTable(    #   
            [["failure", "True", 0.7],
            ["failure", "False", 0.3],
            ["not_failure", "True", 0.0],    
            ["not_failure", "False", 1.0]],
            [self.node_dict["place_failure"]])

        #   pickに失敗したことでどこかにいく確率
        self.node_dict["target_not_exist_by_pick_failure"] = ConditionalProbabilityTable(    #   
            [["failure", "True", 0.8],
            ["failure", "False", 0.2],
            ["not_failure", "True", 0.0],    
            ["not_failure", "False", 1.0]],
            [self.node_dict["pick_failure"]])

        

        self.node_dict["target_not_exist"] = self.generate_OrCPT(["target_not_exist_by_place_failure", "target_not_exist_by_pick_failure"], ["exist", "not_exist"])
        ...
        self.solution_dict["self_occlusion"] = "Spin"
        ...
        #     ["exist", "True", 0.25],
        #     ["exist", "False", 0.75]],
        #     [self.node_dict["target_not_exist"]])    
...
# small_error": 0.5, "medium_error": 0.4, "big_error"
        ...
        self.node_dict["grasp_result"]= self.generate_OrCPT(["grasp_failed_by_recognition_orientation_error", 
                                                        "grasp_failed_by_recognition_position_error"], ["success", "failure"])    #ロボットが衝突する確率
        ...
        # self.node_dict["undetected_error_by_target_not_exist"] = ConditionalProbabilityTable( #   目標が存在しないことが原因で検出されなかった確率
        ...
        self.node_dict["undetected_error_by_occlusion"] = ConditionalProbabilityTable(    #   オクルージョンが原因で検出されなかった確率
            [["occlusion", "True", 0.9],
            ["occlusion", "False", 0.1],
        ...
        self.node_dict["undetected_error_by_target_not_exist"] = ConditionalProbabilityTable(    #   変数をT,Fにしてるだけ
            [["exist", "True", 0.0],
            ["exist", "False", 1.0],
            ["not_exist", "True", 1.0],
            ["not_exist", "False", 0.0]],
        ...
        # self.target_distance_xy = DiscreteDistribution({"under0.4": 0.5, "other": 0.5}) #   baseからの距離
        # self.target_distance_z = DiscreteDistribution({"under0.4": 0.2, "other": 0.8}) #   baseからの距離
        # self.node_dict["undetected_error_by_frame_out"] = DiscreteDistribution({"collision": 0.2, "no_collision": 0.8}) #   

        ConditionalProbabilityTable(    #   変数をT,Fにしてるだけ
            [["exist", "True", 0.0],
            ["exist", "False", 1.0],
            ["not_exist", "True", 1.0],
            ["not_exist", "False", 0.0]],
        ...
        #   フレームアウトしてる可能性があれば少し離れるとか上下を見るとか考えられるけどまだ未実装
        self.node_dict["undetected_error"] = self.generate_OrCPT(["undetected_error_by_yolo", 
                                                                "undetected_error_by_occlusion",
                                                                "undetected_error_by_target_not_exist"], ["no_error", "error"])

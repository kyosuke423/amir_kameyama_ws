...
: Node("object_position_boundingbox")
{
  ...
  objects.header.frame_id = "camera_color_optical_frame"; //pointcloudのフレーム
  ...
  sub_cloud_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
      camera_pointcloud_topic_, rclcpp::SensorDataQoS(),
	...
	for(int i=0; i< (int)boxes->bounding_boxes.size(); i++)
  {
    if(boxes->bounding_boxes[i].class_id == target_label_1)
    {
      ...
      if(depth_is_subscribed_){
        ...
        if(count==0)continue; //  0で割るとnanになるので例外処理。0になる場合が割とある

        vision_msgs::msg::Detection3D object;
        object.id = boxes->bounding_boxes[i].class_id; 
        object.bbox.center.position.x = object_sampling_position.x/count;//カメラ座標系x軸
        object.bbox.center.position.y = object_sampling_position.y/count;//カメラ座標系y軸
        object.bbox.center.position.z = object_sampling_position.z/count;//カメラ座標系z軸

// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from failure_detection_msgs:action/Analysys.idl
// generated code does not contain a copyright notice
#include "failure_detection_msgs/action/detail/analysys__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `node_name`
#include "rosidl_runtime_c/string_functions.h"

bool
failure_detection_msgs__action__Analysys_Goal__init(failure_detection_msgs__action__Analysys_Goal * msg)
{
  if (!msg) {
    return false;
  }
  // node_name
  if (!rosidl_runtime_c__String__init(&msg->node_name)) {
    failure_detection_msgs__action__Analysys_Goal__fini(msg);
    return false;
  }
  // node_id
  return true;
}

void
failure_detection_msgs__action__Analysys_Goal__fini(failure_detection_msgs__action__Analysys_Goal * msg)
{
  if (!msg) {
    return;
  }
  // node_name
  rosidl_runtime_c__String__fini(&msg->node_name);
  // node_id
}

bool
failure_detection_msgs__action__Analysys_Goal__are_equal(const failure_detection_msgs__action__Analysys_Goal * lhs, const failure_detection_msgs__action__Analysys_Goal * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // node_name
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->node_name), &(rhs->node_name)))
  {
    return false;
  }
  // node_id
  if (lhs->node_id != rhs->node_id) {
    return false;
  }
  return true;
}

bool
failure_detection_msgs__action__Analysys_Goal__copy(
  const failure_detection_msgs__action__Analysys_Goal * input,
  failure_detection_msgs__action__Analysys_Goal * output)
{
  if (!input || !output) {
    return false;
  }
  // node_name
  if (!rosidl_runtime_c__String__copy(
      &(input->node_name), &(output->node_name)))
  {
    return false;
  }
  // node_id
  output->node_id = input->node_id;
  return true;
}

failure_detection_msgs__action__Analysys_Goal *
failure_detection_msgs__action__Analysys_Goal__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_Goal * msg = (failure_detection_msgs__action__Analysys_Goal *)allocator.allocate(sizeof(failure_detection_msgs__action__Analysys_Goal), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(failure_detection_msgs__action__Analysys_Goal));
  bool success = failure_detection_msgs__action__Analysys_Goal__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
failure_detection_msgs__action__Analysys_Goal__destroy(failure_detection_msgs__action__Analysys_Goal * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    failure_detection_msgs__action__Analysys_Goal__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
failure_detection_msgs__action__Analysys_Goal__Sequence__init(failure_detection_msgs__action__Analysys_Goal__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_Goal * data = NULL;

  if (size) {
    data = (failure_detection_msgs__action__Analysys_Goal *)allocator.zero_allocate(size, sizeof(failure_detection_msgs__action__Analysys_Goal), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = failure_detection_msgs__action__Analysys_Goal__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        failure_detection_msgs__action__Analysys_Goal__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
failure_detection_msgs__action__Analysys_Goal__Sequence__fini(failure_detection_msgs__action__Analysys_Goal__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      failure_detection_msgs__action__Analysys_Goal__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

failure_detection_msgs__action__Analysys_Goal__Sequence *
failure_detection_msgs__action__Analysys_Goal__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_Goal__Sequence * array = (failure_detection_msgs__action__Analysys_Goal__Sequence *)allocator.allocate(sizeof(failure_detection_msgs__action__Analysys_Goal__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = failure_detection_msgs__action__Analysys_Goal__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
failure_detection_msgs__action__Analysys_Goal__Sequence__destroy(failure_detection_msgs__action__Analysys_Goal__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    failure_detection_msgs__action__Analysys_Goal__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
failure_detection_msgs__action__Analysys_Goal__Sequence__are_equal(const failure_detection_msgs__action__Analysys_Goal__Sequence * lhs, const failure_detection_msgs__action__Analysys_Goal__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!failure_detection_msgs__action__Analysys_Goal__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
failure_detection_msgs__action__Analysys_Goal__Sequence__copy(
  const failure_detection_msgs__action__Analysys_Goal__Sequence * input,
  failure_detection_msgs__action__Analysys_Goal__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(failure_detection_msgs__action__Analysys_Goal);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    failure_detection_msgs__action__Analysys_Goal * data =
      (failure_detection_msgs__action__Analysys_Goal *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!failure_detection_msgs__action__Analysys_Goal__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          failure_detection_msgs__action__Analysys_Goal__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!failure_detection_msgs__action__Analysys_Goal__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


bool
failure_detection_msgs__action__Analysys_Result__init(failure_detection_msgs__action__Analysys_Result * msg)
{
  if (!msg) {
    return false;
  }
  // success_probability
  return true;
}

void
failure_detection_msgs__action__Analysys_Result__fini(failure_detection_msgs__action__Analysys_Result * msg)
{
  if (!msg) {
    return;
  }
  // success_probability
}

bool
failure_detection_msgs__action__Analysys_Result__are_equal(const failure_detection_msgs__action__Analysys_Result * lhs, const failure_detection_msgs__action__Analysys_Result * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // success_probability
  if (lhs->success_probability != rhs->success_probability) {
    return false;
  }
  return true;
}

bool
failure_detection_msgs__action__Analysys_Result__copy(
  const failure_detection_msgs__action__Analysys_Result * input,
  failure_detection_msgs__action__Analysys_Result * output)
{
  if (!input || !output) {
    return false;
  }
  // success_probability
  output->success_probability = input->success_probability;
  return true;
}

failure_detection_msgs__action__Analysys_Result *
failure_detection_msgs__action__Analysys_Result__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_Result * msg = (failure_detection_msgs__action__Analysys_Result *)allocator.allocate(sizeof(failure_detection_msgs__action__Analysys_Result), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(failure_detection_msgs__action__Analysys_Result));
  bool success = failure_detection_msgs__action__Analysys_Result__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
failure_detection_msgs__action__Analysys_Result__destroy(failure_detection_msgs__action__Analysys_Result * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    failure_detection_msgs__action__Analysys_Result__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
failure_detection_msgs__action__Analysys_Result__Sequence__init(failure_detection_msgs__action__Analysys_Result__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_Result * data = NULL;

  if (size) {
    data = (failure_detection_msgs__action__Analysys_Result *)allocator.zero_allocate(size, sizeof(failure_detection_msgs__action__Analysys_Result), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = failure_detection_msgs__action__Analysys_Result__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        failure_detection_msgs__action__Analysys_Result__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
failure_detection_msgs__action__Analysys_Result__Sequence__fini(failure_detection_msgs__action__Analysys_Result__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      failure_detection_msgs__action__Analysys_Result__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

failure_detection_msgs__action__Analysys_Result__Sequence *
failure_detection_msgs__action__Analysys_Result__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_Result__Sequence * array = (failure_detection_msgs__action__Analysys_Result__Sequence *)allocator.allocate(sizeof(failure_detection_msgs__action__Analysys_Result__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = failure_detection_msgs__action__Analysys_Result__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
failure_detection_msgs__action__Analysys_Result__Sequence__destroy(failure_detection_msgs__action__Analysys_Result__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    failure_detection_msgs__action__Analysys_Result__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
failure_detection_msgs__action__Analysys_Result__Sequence__are_equal(const failure_detection_msgs__action__Analysys_Result__Sequence * lhs, const failure_detection_msgs__action__Analysys_Result__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!failure_detection_msgs__action__Analysys_Result__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
failure_detection_msgs__action__Analysys_Result__Sequence__copy(
  const failure_detection_msgs__action__Analysys_Result__Sequence * input,
  failure_detection_msgs__action__Analysys_Result__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(failure_detection_msgs__action__Analysys_Result);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    failure_detection_msgs__action__Analysys_Result * data =
      (failure_detection_msgs__action__Analysys_Result *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!failure_detection_msgs__action__Analysys_Result__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          failure_detection_msgs__action__Analysys_Result__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!failure_detection_msgs__action__Analysys_Result__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


bool
failure_detection_msgs__action__Analysys_Feedback__init(failure_detection_msgs__action__Analysys_Feedback * msg)
{
  if (!msg) {
    return false;
  }
  // success_probability
  return true;
}

void
failure_detection_msgs__action__Analysys_Feedback__fini(failure_detection_msgs__action__Analysys_Feedback * msg)
{
  if (!msg) {
    return;
  }
  // success_probability
}

bool
failure_detection_msgs__action__Analysys_Feedback__are_equal(const failure_detection_msgs__action__Analysys_Feedback * lhs, const failure_detection_msgs__action__Analysys_Feedback * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // success_probability
  if (lhs->success_probability != rhs->success_probability) {
    return false;
  }
  return true;
}

bool
failure_detection_msgs__action__Analysys_Feedback__copy(
  const failure_detection_msgs__action__Analysys_Feedback * input,
  failure_detection_msgs__action__Analysys_Feedback * output)
{
  if (!input || !output) {
    return false;
  }
  // success_probability
  output->success_probability = input->success_probability;
  return true;
}

failure_detection_msgs__action__Analysys_Feedback *
failure_detection_msgs__action__Analysys_Feedback__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_Feedback * msg = (failure_detection_msgs__action__Analysys_Feedback *)allocator.allocate(sizeof(failure_detection_msgs__action__Analysys_Feedback), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(failure_detection_msgs__action__Analysys_Feedback));
  bool success = failure_detection_msgs__action__Analysys_Feedback__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
failure_detection_msgs__action__Analysys_Feedback__destroy(failure_detection_msgs__action__Analysys_Feedback * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    failure_detection_msgs__action__Analysys_Feedback__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
failure_detection_msgs__action__Analysys_Feedback__Sequence__init(failure_detection_msgs__action__Analysys_Feedback__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_Feedback * data = NULL;

  if (size) {
    data = (failure_detection_msgs__action__Analysys_Feedback *)allocator.zero_allocate(size, sizeof(failure_detection_msgs__action__Analysys_Feedback), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = failure_detection_msgs__action__Analysys_Feedback__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        failure_detection_msgs__action__Analysys_Feedback__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
failure_detection_msgs__action__Analysys_Feedback__Sequence__fini(failure_detection_msgs__action__Analysys_Feedback__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      failure_detection_msgs__action__Analysys_Feedback__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

failure_detection_msgs__action__Analysys_Feedback__Sequence *
failure_detection_msgs__action__Analysys_Feedback__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_Feedback__Sequence * array = (failure_detection_msgs__action__Analysys_Feedback__Sequence *)allocator.allocate(sizeof(failure_detection_msgs__action__Analysys_Feedback__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = failure_detection_msgs__action__Analysys_Feedback__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
failure_detection_msgs__action__Analysys_Feedback__Sequence__destroy(failure_detection_msgs__action__Analysys_Feedback__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    failure_detection_msgs__action__Analysys_Feedback__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
failure_detection_msgs__action__Analysys_Feedback__Sequence__are_equal(const failure_detection_msgs__action__Analysys_Feedback__Sequence * lhs, const failure_detection_msgs__action__Analysys_Feedback__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!failure_detection_msgs__action__Analysys_Feedback__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
failure_detection_msgs__action__Analysys_Feedback__Sequence__copy(
  const failure_detection_msgs__action__Analysys_Feedback__Sequence * input,
  failure_detection_msgs__action__Analysys_Feedback__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(failure_detection_msgs__action__Analysys_Feedback);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    failure_detection_msgs__action__Analysys_Feedback * data =
      (failure_detection_msgs__action__Analysys_Feedback *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!failure_detection_msgs__action__Analysys_Feedback__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          failure_detection_msgs__action__Analysys_Feedback__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!failure_detection_msgs__action__Analysys_Feedback__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
#include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `goal`
// already included above
// #include "failure_detection_msgs/action/detail/analysys__functions.h"

bool
failure_detection_msgs__action__Analysys_SendGoal_Request__init(failure_detection_msgs__action__Analysys_SendGoal_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    failure_detection_msgs__action__Analysys_SendGoal_Request__fini(msg);
    return false;
  }
  // goal
  if (!failure_detection_msgs__action__Analysys_Goal__init(&msg->goal)) {
    failure_detection_msgs__action__Analysys_SendGoal_Request__fini(msg);
    return false;
  }
  return true;
}

void
failure_detection_msgs__action__Analysys_SendGoal_Request__fini(failure_detection_msgs__action__Analysys_SendGoal_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // goal
  failure_detection_msgs__action__Analysys_Goal__fini(&msg->goal);
}

bool
failure_detection_msgs__action__Analysys_SendGoal_Request__are_equal(const failure_detection_msgs__action__Analysys_SendGoal_Request * lhs, const failure_detection_msgs__action__Analysys_SendGoal_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // goal
  if (!failure_detection_msgs__action__Analysys_Goal__are_equal(
      &(lhs->goal), &(rhs->goal)))
  {
    return false;
  }
  return true;
}

bool
failure_detection_msgs__action__Analysys_SendGoal_Request__copy(
  const failure_detection_msgs__action__Analysys_SendGoal_Request * input,
  failure_detection_msgs__action__Analysys_SendGoal_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // goal
  if (!failure_detection_msgs__action__Analysys_Goal__copy(
      &(input->goal), &(output->goal)))
  {
    return false;
  }
  return true;
}

failure_detection_msgs__action__Analysys_SendGoal_Request *
failure_detection_msgs__action__Analysys_SendGoal_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_SendGoal_Request * msg = (failure_detection_msgs__action__Analysys_SendGoal_Request *)allocator.allocate(sizeof(failure_detection_msgs__action__Analysys_SendGoal_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(failure_detection_msgs__action__Analysys_SendGoal_Request));
  bool success = failure_detection_msgs__action__Analysys_SendGoal_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
failure_detection_msgs__action__Analysys_SendGoal_Request__destroy(failure_detection_msgs__action__Analysys_SendGoal_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    failure_detection_msgs__action__Analysys_SendGoal_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
failure_detection_msgs__action__Analysys_SendGoal_Request__Sequence__init(failure_detection_msgs__action__Analysys_SendGoal_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_SendGoal_Request * data = NULL;

  if (size) {
    data = (failure_detection_msgs__action__Analysys_SendGoal_Request *)allocator.zero_allocate(size, sizeof(failure_detection_msgs__action__Analysys_SendGoal_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = failure_detection_msgs__action__Analysys_SendGoal_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        failure_detection_msgs__action__Analysys_SendGoal_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
failure_detection_msgs__action__Analysys_SendGoal_Request__Sequence__fini(failure_detection_msgs__action__Analysys_SendGoal_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      failure_detection_msgs__action__Analysys_SendGoal_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

failure_detection_msgs__action__Analysys_SendGoal_Request__Sequence *
failure_detection_msgs__action__Analysys_SendGoal_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_SendGoal_Request__Sequence * array = (failure_detection_msgs__action__Analysys_SendGoal_Request__Sequence *)allocator.allocate(sizeof(failure_detection_msgs__action__Analysys_SendGoal_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = failure_detection_msgs__action__Analysys_SendGoal_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
failure_detection_msgs__action__Analysys_SendGoal_Request__Sequence__destroy(failure_detection_msgs__action__Analysys_SendGoal_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    failure_detection_msgs__action__Analysys_SendGoal_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
failure_detection_msgs__action__Analysys_SendGoal_Request__Sequence__are_equal(const failure_detection_msgs__action__Analysys_SendGoal_Request__Sequence * lhs, const failure_detection_msgs__action__Analysys_SendGoal_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!failure_detection_msgs__action__Analysys_SendGoal_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
failure_detection_msgs__action__Analysys_SendGoal_Request__Sequence__copy(
  const failure_detection_msgs__action__Analysys_SendGoal_Request__Sequence * input,
  failure_detection_msgs__action__Analysys_SendGoal_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(failure_detection_msgs__action__Analysys_SendGoal_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    failure_detection_msgs__action__Analysys_SendGoal_Request * data =
      (failure_detection_msgs__action__Analysys_SendGoal_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!failure_detection_msgs__action__Analysys_SendGoal_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          failure_detection_msgs__action__Analysys_SendGoal_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!failure_detection_msgs__action__Analysys_SendGoal_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `stamp`
#include "builtin_interfaces/msg/detail/time__functions.h"

bool
failure_detection_msgs__action__Analysys_SendGoal_Response__init(failure_detection_msgs__action__Analysys_SendGoal_Response * msg)
{
  if (!msg) {
    return false;
  }
  // accepted
  // stamp
  if (!builtin_interfaces__msg__Time__init(&msg->stamp)) {
    failure_detection_msgs__action__Analysys_SendGoal_Response__fini(msg);
    return false;
  }
  return true;
}

void
failure_detection_msgs__action__Analysys_SendGoal_Response__fini(failure_detection_msgs__action__Analysys_SendGoal_Response * msg)
{
  if (!msg) {
    return;
  }
  // accepted
  // stamp
  builtin_interfaces__msg__Time__fini(&msg->stamp);
}

bool
failure_detection_msgs__action__Analysys_SendGoal_Response__are_equal(const failure_detection_msgs__action__Analysys_SendGoal_Response * lhs, const failure_detection_msgs__action__Analysys_SendGoal_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // accepted
  if (lhs->accepted != rhs->accepted) {
    return false;
  }
  // stamp
  if (!builtin_interfaces__msg__Time__are_equal(
      &(lhs->stamp), &(rhs->stamp)))
  {
    return false;
  }
  return true;
}

bool
failure_detection_msgs__action__Analysys_SendGoal_Response__copy(
  const failure_detection_msgs__action__Analysys_SendGoal_Response * input,
  failure_detection_msgs__action__Analysys_SendGoal_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // accepted
  output->accepted = input->accepted;
  // stamp
  if (!builtin_interfaces__msg__Time__copy(
      &(input->stamp), &(output->stamp)))
  {
    return false;
  }
  return true;
}

failure_detection_msgs__action__Analysys_SendGoal_Response *
failure_detection_msgs__action__Analysys_SendGoal_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_SendGoal_Response * msg = (failure_detection_msgs__action__Analysys_SendGoal_Response *)allocator.allocate(sizeof(failure_detection_msgs__action__Analysys_SendGoal_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(failure_detection_msgs__action__Analysys_SendGoal_Response));
  bool success = failure_detection_msgs__action__Analysys_SendGoal_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
failure_detection_msgs__action__Analysys_SendGoal_Response__destroy(failure_detection_msgs__action__Analysys_SendGoal_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    failure_detection_msgs__action__Analysys_SendGoal_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
failure_detection_msgs__action__Analysys_SendGoal_Response__Sequence__init(failure_detection_msgs__action__Analysys_SendGoal_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_SendGoal_Response * data = NULL;

  if (size) {
    data = (failure_detection_msgs__action__Analysys_SendGoal_Response *)allocator.zero_allocate(size, sizeof(failure_detection_msgs__action__Analysys_SendGoal_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = failure_detection_msgs__action__Analysys_SendGoal_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        failure_detection_msgs__action__Analysys_SendGoal_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
failure_detection_msgs__action__Analysys_SendGoal_Response__Sequence__fini(failure_detection_msgs__action__Analysys_SendGoal_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      failure_detection_msgs__action__Analysys_SendGoal_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

failure_detection_msgs__action__Analysys_SendGoal_Response__Sequence *
failure_detection_msgs__action__Analysys_SendGoal_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_SendGoal_Response__Sequence * array = (failure_detection_msgs__action__Analysys_SendGoal_Response__Sequence *)allocator.allocate(sizeof(failure_detection_msgs__action__Analysys_SendGoal_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = failure_detection_msgs__action__Analysys_SendGoal_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
failure_detection_msgs__action__Analysys_SendGoal_Response__Sequence__destroy(failure_detection_msgs__action__Analysys_SendGoal_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    failure_detection_msgs__action__Analysys_SendGoal_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
failure_detection_msgs__action__Analysys_SendGoal_Response__Sequence__are_equal(const failure_detection_msgs__action__Analysys_SendGoal_Response__Sequence * lhs, const failure_detection_msgs__action__Analysys_SendGoal_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!failure_detection_msgs__action__Analysys_SendGoal_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
failure_detection_msgs__action__Analysys_SendGoal_Response__Sequence__copy(
  const failure_detection_msgs__action__Analysys_SendGoal_Response__Sequence * input,
  failure_detection_msgs__action__Analysys_SendGoal_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(failure_detection_msgs__action__Analysys_SendGoal_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    failure_detection_msgs__action__Analysys_SendGoal_Response * data =
      (failure_detection_msgs__action__Analysys_SendGoal_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!failure_detection_msgs__action__Analysys_SendGoal_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          failure_detection_msgs__action__Analysys_SendGoal_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!failure_detection_msgs__action__Analysys_SendGoal_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"

bool
failure_detection_msgs__action__Analysys_GetResult_Request__init(failure_detection_msgs__action__Analysys_GetResult_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    failure_detection_msgs__action__Analysys_GetResult_Request__fini(msg);
    return false;
  }
  return true;
}

void
failure_detection_msgs__action__Analysys_GetResult_Request__fini(failure_detection_msgs__action__Analysys_GetResult_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
}

bool
failure_detection_msgs__action__Analysys_GetResult_Request__are_equal(const failure_detection_msgs__action__Analysys_GetResult_Request * lhs, const failure_detection_msgs__action__Analysys_GetResult_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  return true;
}

bool
failure_detection_msgs__action__Analysys_GetResult_Request__copy(
  const failure_detection_msgs__action__Analysys_GetResult_Request * input,
  failure_detection_msgs__action__Analysys_GetResult_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  return true;
}

failure_detection_msgs__action__Analysys_GetResult_Request *
failure_detection_msgs__action__Analysys_GetResult_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_GetResult_Request * msg = (failure_detection_msgs__action__Analysys_GetResult_Request *)allocator.allocate(sizeof(failure_detection_msgs__action__Analysys_GetResult_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(failure_detection_msgs__action__Analysys_GetResult_Request));
  bool success = failure_detection_msgs__action__Analysys_GetResult_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
failure_detection_msgs__action__Analysys_GetResult_Request__destroy(failure_detection_msgs__action__Analysys_GetResult_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    failure_detection_msgs__action__Analysys_GetResult_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
failure_detection_msgs__action__Analysys_GetResult_Request__Sequence__init(failure_detection_msgs__action__Analysys_GetResult_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_GetResult_Request * data = NULL;

  if (size) {
    data = (failure_detection_msgs__action__Analysys_GetResult_Request *)allocator.zero_allocate(size, sizeof(failure_detection_msgs__action__Analysys_GetResult_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = failure_detection_msgs__action__Analysys_GetResult_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        failure_detection_msgs__action__Analysys_GetResult_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
failure_detection_msgs__action__Analysys_GetResult_Request__Sequence__fini(failure_detection_msgs__action__Analysys_GetResult_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      failure_detection_msgs__action__Analysys_GetResult_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

failure_detection_msgs__action__Analysys_GetResult_Request__Sequence *
failure_detection_msgs__action__Analysys_GetResult_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_GetResult_Request__Sequence * array = (failure_detection_msgs__action__Analysys_GetResult_Request__Sequence *)allocator.allocate(sizeof(failure_detection_msgs__action__Analysys_GetResult_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = failure_detection_msgs__action__Analysys_GetResult_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
failure_detection_msgs__action__Analysys_GetResult_Request__Sequence__destroy(failure_detection_msgs__action__Analysys_GetResult_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    failure_detection_msgs__action__Analysys_GetResult_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
failure_detection_msgs__action__Analysys_GetResult_Request__Sequence__are_equal(const failure_detection_msgs__action__Analysys_GetResult_Request__Sequence * lhs, const failure_detection_msgs__action__Analysys_GetResult_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!failure_detection_msgs__action__Analysys_GetResult_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
failure_detection_msgs__action__Analysys_GetResult_Request__Sequence__copy(
  const failure_detection_msgs__action__Analysys_GetResult_Request__Sequence * input,
  failure_detection_msgs__action__Analysys_GetResult_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(failure_detection_msgs__action__Analysys_GetResult_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    failure_detection_msgs__action__Analysys_GetResult_Request * data =
      (failure_detection_msgs__action__Analysys_GetResult_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!failure_detection_msgs__action__Analysys_GetResult_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          failure_detection_msgs__action__Analysys_GetResult_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!failure_detection_msgs__action__Analysys_GetResult_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `result`
// already included above
// #include "failure_detection_msgs/action/detail/analysys__functions.h"

bool
failure_detection_msgs__action__Analysys_GetResult_Response__init(failure_detection_msgs__action__Analysys_GetResult_Response * msg)
{
  if (!msg) {
    return false;
  }
  // status
  // result
  if (!failure_detection_msgs__action__Analysys_Result__init(&msg->result)) {
    failure_detection_msgs__action__Analysys_GetResult_Response__fini(msg);
    return false;
  }
  return true;
}

void
failure_detection_msgs__action__Analysys_GetResult_Response__fini(failure_detection_msgs__action__Analysys_GetResult_Response * msg)
{
  if (!msg) {
    return;
  }
  // status
  // result
  failure_detection_msgs__action__Analysys_Result__fini(&msg->result);
}

bool
failure_detection_msgs__action__Analysys_GetResult_Response__are_equal(const failure_detection_msgs__action__Analysys_GetResult_Response * lhs, const failure_detection_msgs__action__Analysys_GetResult_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // status
  if (lhs->status != rhs->status) {
    return false;
  }
  // result
  if (!failure_detection_msgs__action__Analysys_Result__are_equal(
      &(lhs->result), &(rhs->result)))
  {
    return false;
  }
  return true;
}

bool
failure_detection_msgs__action__Analysys_GetResult_Response__copy(
  const failure_detection_msgs__action__Analysys_GetResult_Response * input,
  failure_detection_msgs__action__Analysys_GetResult_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // status
  output->status = input->status;
  // result
  if (!failure_detection_msgs__action__Analysys_Result__copy(
      &(input->result), &(output->result)))
  {
    return false;
  }
  return true;
}

failure_detection_msgs__action__Analysys_GetResult_Response *
failure_detection_msgs__action__Analysys_GetResult_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_GetResult_Response * msg = (failure_detection_msgs__action__Analysys_GetResult_Response *)allocator.allocate(sizeof(failure_detection_msgs__action__Analysys_GetResult_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(failure_detection_msgs__action__Analysys_GetResult_Response));
  bool success = failure_detection_msgs__action__Analysys_GetResult_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
failure_detection_msgs__action__Analysys_GetResult_Response__destroy(failure_detection_msgs__action__Analysys_GetResult_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    failure_detection_msgs__action__Analysys_GetResult_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
failure_detection_msgs__action__Analysys_GetResult_Response__Sequence__init(failure_detection_msgs__action__Analysys_GetResult_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_GetResult_Response * data = NULL;

  if (size) {
    data = (failure_detection_msgs__action__Analysys_GetResult_Response *)allocator.zero_allocate(size, sizeof(failure_detection_msgs__action__Analysys_GetResult_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = failure_detection_msgs__action__Analysys_GetResult_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        failure_detection_msgs__action__Analysys_GetResult_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
failure_detection_msgs__action__Analysys_GetResult_Response__Sequence__fini(failure_detection_msgs__action__Analysys_GetResult_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      failure_detection_msgs__action__Analysys_GetResult_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

failure_detection_msgs__action__Analysys_GetResult_Response__Sequence *
failure_detection_msgs__action__Analysys_GetResult_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_GetResult_Response__Sequence * array = (failure_detection_msgs__action__Analysys_GetResult_Response__Sequence *)allocator.allocate(sizeof(failure_detection_msgs__action__Analysys_GetResult_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = failure_detection_msgs__action__Analysys_GetResult_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
failure_detection_msgs__action__Analysys_GetResult_Response__Sequence__destroy(failure_detection_msgs__action__Analysys_GetResult_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    failure_detection_msgs__action__Analysys_GetResult_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
failure_detection_msgs__action__Analysys_GetResult_Response__Sequence__are_equal(const failure_detection_msgs__action__Analysys_GetResult_Response__Sequence * lhs, const failure_detection_msgs__action__Analysys_GetResult_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!failure_detection_msgs__action__Analysys_GetResult_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
failure_detection_msgs__action__Analysys_GetResult_Response__Sequence__copy(
  const failure_detection_msgs__action__Analysys_GetResult_Response__Sequence * input,
  failure_detection_msgs__action__Analysys_GetResult_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(failure_detection_msgs__action__Analysys_GetResult_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    failure_detection_msgs__action__Analysys_GetResult_Response * data =
      (failure_detection_msgs__action__Analysys_GetResult_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!failure_detection_msgs__action__Analysys_GetResult_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          failure_detection_msgs__action__Analysys_GetResult_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!failure_detection_msgs__action__Analysys_GetResult_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `feedback`
// already included above
// #include "failure_detection_msgs/action/detail/analysys__functions.h"

bool
failure_detection_msgs__action__Analysys_FeedbackMessage__init(failure_detection_msgs__action__Analysys_FeedbackMessage * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    failure_detection_msgs__action__Analysys_FeedbackMessage__fini(msg);
    return false;
  }
  // feedback
  if (!failure_detection_msgs__action__Analysys_Feedback__init(&msg->feedback)) {
    failure_detection_msgs__action__Analysys_FeedbackMessage__fini(msg);
    return false;
  }
  return true;
}

void
failure_detection_msgs__action__Analysys_FeedbackMessage__fini(failure_detection_msgs__action__Analysys_FeedbackMessage * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // feedback
  failure_detection_msgs__action__Analysys_Feedback__fini(&msg->feedback);
}

bool
failure_detection_msgs__action__Analysys_FeedbackMessage__are_equal(const failure_detection_msgs__action__Analysys_FeedbackMessage * lhs, const failure_detection_msgs__action__Analysys_FeedbackMessage * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // feedback
  if (!failure_detection_msgs__action__Analysys_Feedback__are_equal(
      &(lhs->feedback), &(rhs->feedback)))
  {
    return false;
  }
  return true;
}

bool
failure_detection_msgs__action__Analysys_FeedbackMessage__copy(
  const failure_detection_msgs__action__Analysys_FeedbackMessage * input,
  failure_detection_msgs__action__Analysys_FeedbackMessage * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // feedback
  if (!failure_detection_msgs__action__Analysys_Feedback__copy(
      &(input->feedback), &(output->feedback)))
  {
    return false;
  }
  return true;
}

failure_detection_msgs__action__Analysys_FeedbackMessage *
failure_detection_msgs__action__Analysys_FeedbackMessage__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_FeedbackMessage * msg = (failure_detection_msgs__action__Analysys_FeedbackMessage *)allocator.allocate(sizeof(failure_detection_msgs__action__Analysys_FeedbackMessage), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(failure_detection_msgs__action__Analysys_FeedbackMessage));
  bool success = failure_detection_msgs__action__Analysys_FeedbackMessage__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
failure_detection_msgs__action__Analysys_FeedbackMessage__destroy(failure_detection_msgs__action__Analysys_FeedbackMessage * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    failure_detection_msgs__action__Analysys_FeedbackMessage__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
failure_detection_msgs__action__Analysys_FeedbackMessage__Sequence__init(failure_detection_msgs__action__Analysys_FeedbackMessage__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_FeedbackMessage * data = NULL;

  if (size) {
    data = (failure_detection_msgs__action__Analysys_FeedbackMessage *)allocator.zero_allocate(size, sizeof(failure_detection_msgs__action__Analysys_FeedbackMessage), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = failure_detection_msgs__action__Analysys_FeedbackMessage__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        failure_detection_msgs__action__Analysys_FeedbackMessage__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
failure_detection_msgs__action__Analysys_FeedbackMessage__Sequence__fini(failure_detection_msgs__action__Analysys_FeedbackMessage__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      failure_detection_msgs__action__Analysys_FeedbackMessage__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

failure_detection_msgs__action__Analysys_FeedbackMessage__Sequence *
failure_detection_msgs__action__Analysys_FeedbackMessage__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  failure_detection_msgs__action__Analysys_FeedbackMessage__Sequence * array = (failure_detection_msgs__action__Analysys_FeedbackMessage__Sequence *)allocator.allocate(sizeof(failure_detection_msgs__action__Analysys_FeedbackMessage__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = failure_detection_msgs__action__Analysys_FeedbackMessage__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
failure_detection_msgs__action__Analysys_FeedbackMessage__Sequence__destroy(failure_detection_msgs__action__Analysys_FeedbackMessage__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    failure_detection_msgs__action__Analysys_FeedbackMessage__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
failure_detection_msgs__action__Analysys_FeedbackMessage__Sequence__are_equal(const failure_detection_msgs__action__Analysys_FeedbackMessage__Sequence * lhs, const failure_detection_msgs__action__Analysys_FeedbackMessage__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!failure_detection_msgs__action__Analysys_FeedbackMessage__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
failure_detection_msgs__action__Analysys_FeedbackMessage__Sequence__copy(
  const failure_detection_msgs__action__Analysys_FeedbackMessage__Sequence * input,
  failure_detection_msgs__action__Analysys_FeedbackMessage__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(failure_detection_msgs__action__Analysys_FeedbackMessage);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    failure_detection_msgs__action__Analysys_FeedbackMessage * data =
      (failure_detection_msgs__action__Analysys_FeedbackMessage *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!failure_detection_msgs__action__Analysys_FeedbackMessage__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          failure_detection_msgs__action__Analysys_FeedbackMessage__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!failure_detection_msgs__action__Analysys_FeedbackMessage__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
